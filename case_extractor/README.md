...
박사논문 판례분석용 프로그램입니다. **판결문(PDF/DOCX/DOC)** 을 넣으면 `코딩시트.xlsx`
...
Python 설치 없이 더블클릭만으로 쓰고 싶다면, 맥은 "맥용 더블클릭 실행 앱(.app) 만들기",
윈도우는 "윈도우용 더블클릭 실행파일(.exe) 만들기" 섹션을 참고하세요. 둘 다 매번
같은 코드에서 자동으로 만들어지므로, 팀 안에서 맥 쓰는 사람과 윈도우 쓰는 사람이
각자 자기 OS용 파일만 받아서 똑같이 쓸 수 있습니다.
...
## 맥용 더블클릭 실행 앱(.app) 만들기

### 방법 A: GitHub에서 자동으로 빌드된 앱 받기 (추천)

Windows exe와 마찬가지로, 이 저장소는 GitHub Actions의 실제 macOS 서버에서 앱을
자동으로 빌드하고 **Releases**의 `macos-app-latest`에 올리도록 설정되어 있습니다.

1. 저장소 페이지의 **Releases** (또는 `저장소주소/releases/tag/macos-app-latest`)로 들어갑니다.
2. **Assets** 목록에서 `PanryeCodingConverter-v숫자-macOS.zip`을 다운로드합니다.
3. 압축을 풀면 `PanryeCodingConverter-v숫자.app`이 나옵니다. 더블클릭하면 실행됩니다.
4. **"확인되지 않은 개발자입니다" 경고가 뜨면**: 앱을 Apple 공식 인증 없이 직접
   빌드했기 때문에 나오는 정상적인 경고입니다. Finder에서 그 앱을 **우클릭(또는
   control+클릭) → 열기**를 선택하고, 뜨는 창에서 다시 "열기"를 누르면 실행됩니다
   (한 번만 해두면 이후에는 그냥 더블클릭으로 열립니다).

Windows와 마찬가지로 `macos-app-v1`, `macos-app-v2`, ... 처럼 버전별 릴리스도
영구히 남아있어서, 필요하면 이전 버전을 다시 받을 수 있습니다.

### 방법 B: 내 맥에서 직접 빌드

1. 이 저장소를 내려받습니다 (`git clone` 또는 ZIP 다운로드).
2. 터미널에서 `case_extractor` 폴더로 이동한 뒤 실행:
   ```bash
   bash build_macos_app.sh
   ```
3. 완료되면 `dist/PanryeCodingConverter.app`이 생성됩니다. 더블클릭해서 실행하세요
   (마찬가지로 "확인되지 않은 개발자" 경고가 뜨면 우클릭 → 열기).
...
**이전 버전도 계속 남아있습니다.** 빌드할 때마다 `windows-exe-latest`(항상 최신을
가리킴)와는 별도로 `windows-exe-v1`, `windows-exe-v2`, ... 처럼 버전이 매겨진
릴리스도 함께 만들어지고, 이건 새 버전이 나와도 삭제되지 않습니다. 특정 시점 버전을
다시 받고 싶으면 저장소의 **Releases** 목록 전체(`저장소주소/releases`)에서
원하는 버전을 찾아 받으면 됩니다.
...
1. "판결문 입력"에서 **폴더에서 전체 선택** 또는 **파일 개별 선택**(여러 개 동시 선택 가능)으로
   PDF/DOCX/DOC 파일을 추가합니다. 두 방식을 섞어서 계속 추가할 수도 있고, "비우기"로 초기화할 수 있습니다.
...
화면 위쪽의 **"✂️ 수동 모드"** 버튼을 눌러 진행합니다. Anthropic API 키(유료,
...
2. **프롬프트 파일 만들기**: 저장할 폴더를 고르고 버튼을 누르면, 판결문마다
   ...
   들어있고, claude.ai에게 "파일로 답변을 만들 수 있다면 파일명을 `판결문1.json`처럼
   판결문과 똑같이 해달라"는 안내도 함께 포함되어 있습니다. claude.ai가 이 안내를
   따라 파일로 응답하면, **다운로드한 파일 이름을 바꾸지 않고 그대로 써도** 3번
   단계에서 자동으로 인식됩니다 (claude.ai가 항상 이 안내를 완벽히 지키는 것은
   아니라서, 이름이 다르게 나온다면 아래 "이미 있는 JSON 파일 추가" 버튼이나
   붙여넣기 방식을 이용하세요).
...
"엑셀로 합치기"를 눌렀는데 아무 반응이 없거나 실패한다면, 화면 오른쪽의 **"현재
작업 요약"**(완료/건너뜀/실패 건수)과 그 아래 **"진행 상황"** 로그를 먼저 확인하세요.
"건너뜀"이나 "실패" 건수가 있으면, 로그에 각 판결문이 왜 그렇게 됐는지 표시됩니다.
자주 있는 원인 3가지:
...
- `.docx`, `.doc`: 모두 지원합니다 (옛 Word 97-2003 `.doc` 포함).
