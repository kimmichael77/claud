...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--skip-judgment-check",
        action="store_true",
        help="판결문 여부 사전 검사를 건너뛴다 (권장하지 않음)",
    )
    return p
...
def process_file(
    ...
    *,
    coder_id: str,
    model: str | None,
    skip_judgment_check: bool = False,
) -> dict:
    ...
    if not skip_judgment_check:
        ...
        if not ok:
            raise NotJudgmentLikelyError(path.name)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.input_dir is not None:
        ...
        if missing:
            print(f"오류: 다음 파일을 찾을 수 없습니다: {', '.join(missing)}", file=sys.stderr)
            return 1
    ...
    for i, path in enumerate(files, start=1):
        ...
        try:
            row = process_file(
                ...
                model=args.model,
                skip_judgment_check=args.skip_judgment_check,
            )
            rows.append(row)
